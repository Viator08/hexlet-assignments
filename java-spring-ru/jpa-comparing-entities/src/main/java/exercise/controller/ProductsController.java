...
public class ProductsController {
    ...
    public ResponseEntity<Product> create(@RequestBody Product productData) {
        var product = productRepository.findAll()
                .stream()
                .filter(p -> p.equals(productData)).findAny();
        
        if (product.isPresent()) {
            throw new ResourceAlreadyExistsException("Product with title " + productData.getTitle() + " already exists");
        }
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(productRepository.save(productData));
    }
    // END
